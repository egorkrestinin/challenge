...
class EntityHandler implements SubscribingHandlerInterface
{
    ...
    public function serializeEntity(VisitorInterface $visitor, $entity, array $type, Context $context)
    {
        ...
        if (is_int($id) || is_string($id)) {
            ...
            return $visitor->visitString($id->toString(), $type, $context);
        } else {
            throw new InvalidArgumentException(
                sprintf(
                    "Invalid primary key type for entity '%s' (only integer, 
                    string and \\Ramsey\\Uuid\\UuidInterface are supported",
                    $entityClass
                )
            );
        }
     ...
     * @return null|object
     */
    public function deserializeEntity(GenericDeserializationVisitor $visitor, string $id, array $type)
    {
        if (null === $id) {
            return null;
        }
     ...
     * @return string
     */
    protected function getEntityClassFromParameters(array $parameters) : string
    {
     ...
     */
    protected function getEntityManager(string $entityClass) : EntityManagerInterface
    {
